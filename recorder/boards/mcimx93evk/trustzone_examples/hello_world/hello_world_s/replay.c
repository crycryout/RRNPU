...
void replay_handle_interrupt(void)
{
    for (int i = INTERRUPT_HANDLING_START; i < INTERRUPT_HANDLING_END; ++i) {
        ...
        if (rec->op_type == REG_OP_WRITE) {
            *addr = rec->reg_value;
            PRINTF("IRQ WRITE: Addr=%p, Val=0x%08x, Order=%u\n",
            ...
            do {
                v = *addr;
                if (v != expected) {
                    PRINTF("IRQ POLL: Addr=%p, Got=0x%08x, Expect=0x%08x, Order=%u\n",
                             addr, v, expected, rec->op_order);
                }
            } while (v != expected);

            PRINTF("IRQ READ OK: Addr=%p, Val=0x%08x, Order=%u\n",
                     addr, v, rec->op_order);
        }
    }

    /* —— 重放完 48 条之后，把结果数据 dump 出来 —— */
    PRINTF("INFERENCE RESULT (128 bytes):");
    PRINTF("print_result:");
    ...
    for (int i = INIT_VERIFICATION_START; i <= INIT_VERIFICATION_END; ++i) {
            if (i != WAIT) {
                ...
                if (register_access(&register_access_records[i]) != 0) {
                    PRINTF("failed to initialize Ethos\n");
                }
            }
